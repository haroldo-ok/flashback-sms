...
/* fbdump: records which opcodes actually execute (added for the SMS port, GPL) */
OpcodeHookProc g_opcodeHook = 0;
int g_traceObjIndex = -1;   /* fbdump: object currently being processed */
#include "resource.h"
...
int Game::pge_execute(LivePGE *live_pge, const InitPGE *init_pge, const Object *obj) {
	...
	if (obj->opcode1) {
		...
		if (g_opcodeHook) g_opcodeHook(1, obj->opcode1);
	...
	if (obj->opcode2) {
		...
		if (g_opcodeHook) g_opcodeHook(2, obj->opcode2);
	...
	if (obj->opcode3) {
		...
		if (g_opcodeHook) g_opcodeHook(3, obj->opcode3);
