...
/* fbdump: per game frame trace hook (added for the SMS port, GPL) */
FrameHookProc g_frameHook = 0;
int g_drawPgeIndex = -1, g_drawAnimNumber = 0, g_drawPgeX = 0, g_drawPgeY = 0, g_drawPgeFlags = 0;
...
void Game::mainLoop() {
	...
	_vid.updateScreen();
	if (g_frameHook) g_frameHook(this); /* fbdump trace hook */
	updateTiming();
	drawStoryTexts();
...
void Game::drawAnimBuffer(uint8_t stateNum, AnimBufferState *state) {
	...
	if (lastPos != 0xFF) {
		...
		do {
			...
			/* fbdump: tag the pieces about to be drawn with the object that
			 * owns them, so the port can key sprites by animation number */
			g_drawPgeIndex = pge->index;
			g_drawAnimNumber = pge->anim_number;
			g_drawPgeX = pge->pos_x;
			g_drawPgeY = pge->pos_y;
			g_drawPgeFlags = pge->flags;
			if (!(pge->flags & 8)) {
