...
def room_dicts():
    """the per-map compact tile dictionaries, as descriptors for tilepack.fetch"""
    import re
    src = open(f'{GEN}/data_index.c').read()
    body = src[src.index('room_dicts[] = {') + 16:]
    body = body[:body.index('};')]
    out = []
    for m in re.finditer(r'\{([^}]*)\}', body):
        v = [int(x) for x in m.group(1).split(',')]
        out.append(dict(bank0=v[0], bank1=v[1], bank2=v[2], bank3=v[3],
                        start1=v[4], start2=v[5], start3=v[6]))
    return out
...
def clip_frames(t, clip):
    ...
    imgs = [img.copy() for _, _, img in M.play_clip(BLOB, bank, addr, M.fmv_desc_from(defines()))]
...
def room_layers(t, index):
    ...
    desc = room_dicts()[t['room_dict'][index]]
    at = lambda bk, off, k: BLOB[(bk - M.BANK0) * M.BANK + off:(bk - M.BANK0) * M.BANK + off + k]
    ids = BLOB[o + 1810:o + 1810 + n * 2]
    for i in range(n):
        tid = ids[2 * i] | (ids[2 * i + 1] << 8)
        vram[448 - n + i] = M.unplanar(TP.fetch(at, desc, tid))
...
def gen():
    ...
    # only what this build actually contains
    intro = [c for c in (0x40, 0x0D) if c in ids]        # the game intro
    c40 = ids.index(intro[0])
    c0d = ids.index(intro[1]) if len(intro) > 1 else c40
    c00 = ids.index(0x00) if 0x00 in ids else c40        # level 1's own intro
    f40, u40 = clip_frames(t, c40)
    f0d, _ = clip_frames(t, c0d)
    f00, u00 = clip_frames(t, c00)
    shots = [(ids[c40], k) for k in motion_ticks(f40)]
    if c0d != c40:
        shots += [(ids[c0d], k) for k in motion_ticks(f0d, hi=min(1050, len(f0d) - 10))]
    CUT_SHOTS[:] = shots
    ...
    L += ['', 'echo timing: after 1200 real frames of the intro the stream is still in sync (bounded lag)',
          f'waituntil fmv_tick >= {min(1100, len(f0d) - 300)} 1500', 'expect ticks_behind <= 48',
          'run 60', 'expect ticks_behind <= 48',
          'expect fmv_active == 1 w1', 'print fmv_tick']
    L += ['', f'echo logos play to their natural end: {len(f40)} ticks, {u40} uploads (host decode)',
          ('waituntil seq_pos == 1 2000 w1' if c0d != c40 else 'waituntil game_state == 1 3000 w1'),
          f'expect last_clip_ticks == {len(f40)}', f'expect last_clip_uploads == {u40}'] + \
         (['expect fmv_active == 1 w1'] if c0d != c40 else [])
    for cid, tick in CUT_SHOTS:
        if cid != ids[c0d] or c0d == c40: continue
        L += [f'waituntil fmv_tick >= {tick} 3000', f'screenshot shots/sms/cut_{cid:02X}_{tick}.ppm', 'print fmv_tick']
    # with a one-clip intro there is nothing left to skip: it ends at the title
    L += ['', 'echo the intro ends at the title screen'] + (
        ['echo (button 1 skips the rest of it)', 'hold 1', 'run 20', 'hold none']
        if c0d != c40 else []) + [
          'waituntil game_state == 1 3000 w1', 'run 5',
    ...
    import json
    lv_files = json.load(open(f'{GEN}/manifest.json'))['levels']    # what was packed
    if lv_files and 'pge_cnt' in syms:
        ...
        for i, f in enumerate(lv_files):
            ...
            L += [f'expect 0x{syms["pge_cnt"] + i * 2:04X} == {min(d["npges"], 255)}',
...
def compare():
    ...
    if exp.get('title'):
        ...
        # the level-select text is drawn over the picture at run time (row 23,
        # ten cells from column 2), so it is not part of the reference
        m = np.ones(ref.shape[:2], bool)
        m[184:192, 16:96] = False
        bad = int(((ref != shot).any(-1) & m).sum())
        print(f'title screen: differing pixels {bad} (level-select text excluded)')
        ok &= bad == 0
        sheet.append(shot)
